...
export const AchievementsPage = () => {
  ...
  const startLabel = firstRunAt ? formatDate(firstRunAt, language) : "--";
  ...
  return (
    <div className="space-y-4">
      <section className="relative z-0 flex justify-center">
        <div className="w-full max-w-[360px] px-5 py-2 text-center">
          <div className="mx-auto flex h-52 w-full items-center justify-center overflow-hidden">
            <CalendarMedalIcon
              asset={calendarAchievement}
              days={totalRunDays}
              startLabel={startLabel}
              startCaption={language === "zh" ? "开始" : "Start"}
              daysCaption={language === "zh" ? "天" : "days"}
            />
          </div>
        </div>
      </section>

      <div className="relative z-10 mx-auto h-px w-[82%] max-w-[320px] -translate-y-[30px] bg-gradient-to-r from-transparent via-[#c9d8c9]/80 to-transparent" />

      <section className="relative z-10">
        <div className="p-1">
          <div className="px-1 pr-2">
            <div className="relative mx-auto h-[500px] max-w-[360px] overflow-hidden">
              <div
                className="relative h-full touch-none select-none"
                {...deckHandlers}
              >
                <div className="pointer-events-none absolute inset-x-0 bottom-0 z-40 h-24 bg-gradient-to-t from-[#f7f5ef] to-transparent" />
                {wheelCountrySets.map(({ set, index, offset }) => {
                  ...
                  return (
                    <div
                      key={`${set.country}-card-wheel-${offset}`}
                      onClick={() => setActiveCountryIndex(index)}
                      className={cn(
                        "absolute left-0 top-1/2 w-full will-change-transform",
                        isDragging ? "transition-none" : "transition-all duration-300 ease-out",
                        isActiveCard ? "opacity-100" : "opacity-65",
      ...
      {selectedSet ? (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-ink/42 px-4 backdrop-blur-sm">
          <div className="max-h-[86vh] w-full max-w-[760px] overflow-hidden rounded-[8px] bg-[#f7f5ef] shadow-[0_26px_80px_rgba(17,31,22,0.32)] ring-1 ring-white/80">
            <div className="flex justify-end border-b border-sage-100 bg-white px-5 py-4">
              <button
                type="button"
