...
const LandmarkMesh = ({
  ...
  const color = getLandmarkColor(asset.id);
  const shape = getLandmarkShape(asset.id);
  const scale = item.scale;
  const height = 0.95 * scale;
  const labelHeight = modelConfig ? 2.15 * scale : height + 0.75;
  return (
    <group position={[x, y, z]} onClick={(event) => {
      ...
      {!modelConfig ? (
        <mesh position={[0, -0.26, 0]} rotation={[-Math.PI / 2, 0, 0]}>
          <ringGeometry args={[0.44 * scale, 0.58 * scale, 36]} />
          <meshBasicMaterial color={selected ? "#8da58f" : "#c7d4c8"} transparent opacity={selected ? 0.95 : 0.45} />
        </mesh>
      ) : null}

      {!modelConfig ? (
        <mesh castShadow receiveShadow position={[0, -0.05, 0]}>
          <cylinderGeometry args={[0.4 * scale, 0.52 * scale, 0.24, 24]} />
          <meshStandardMaterial color="#efe8db" roughness={0.8} metalness={0.05} />
        </mesh>
      ) : null}
      ...
      {!modelConfig && shape === "box" ? (
        <mesh castShadow receiveShadow position={[0, height / 2 + 0.1, 0]}>
          <boxGeometry args={[0.52 * scale, height, 0.52 * scale]} />
          <meshStandardMaterial color={color} roughness={0.35} metalness={0.08} />
        </mesh>
      ) : null}

      {!modelConfig && shape === "cylinder" ? (
        <mesh castShadow receiveShadow position={[0, height / 2 + 0.08, 0]}>
          <cylinderGeometry args={[0.28 * scale, 0.34 * scale, height + 0.1, 24]} />
          <meshStandardMaterial color={color} roughness={0.35} metalness={0.08} />
        </mesh>
      ) : null}

      {!modelConfig && shape === "cone" ? (
        <mesh castShadow receiveShadow position={[0, height / 2 + 0.08, 0]}>
          <coneGeometry args={[0.38 * scale, height + 0.16, 24]} />
          <meshStandardMaterial color={color} roughness={0.35} metalness={0.08} />
        </mesh>
      ) : null}

      {!modelConfig && shape === "capsule" ? (
        <mesh castShadow receiveShadow position={[0, height / 2 + 0.08, 0]}>
          <capsuleGeometry args={[0.24 * scale, height * 0.7, 5, 12]} />
          <meshStandardMaterial color={color} roughness={0.35} metalness={0.08} />
        </mesh>
      ) : null}
        ...
        {placedLandmarks.map((item) => {
          ...
          return (
            <LandmarkMesh
              ...
              modelConfig={landmarkModelMap[asset.id]}
            />
          );
        })}
      </group>
