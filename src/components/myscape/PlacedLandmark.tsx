...
export const PlacedLandmark = ({ asset, item, selected, onPointerDown, onSelect, onReturnToBox }: PlacedLandmarkProps) => (
  <div
    className={cn(
      "absolute touch-none select-none transition duration-200",
      selected ? "z-30" : "z-10",
    )}
    style={{
      left: item.x,
  ...
  >
    {selected && onReturnToBox ? (
      <button
        type="button"
        onClick={(event) => {
          event.stopPropagation();
          onReturnToBox(item.id);
        }}
        className="absolute -right-1 -top-1 z-40 flex h-7 w-7 items-center justify-center rounded-full bg-white text-sage-700 shadow-[0_10px_20px_rgba(36,50,40,0.18)] ring-1 ring-sage-900/10"
        aria-label={`Return ${asset.name} to box`}
      >
    ...
    <button
      ...
      className="block rounded-[26px] text-left"
